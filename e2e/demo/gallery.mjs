...
export async function gallery(directory, baseURL) {
  return record('design-system', directory, galleryLines, async ({ page, say, expect }) => {
    ...
    await page.getByRole('heading', { name: '01 / Colour' }).scrollIntoViewIfNeeded(); await say(1, 'Colour');
    await page.getByRole('heading', { name: '02 / Typography' }).scrollIntoViewIfNeeded(); await say(2, 'Typography');
    const primary = page.getByRole('button', { name: 'Primary action', exact: true });
    await primary.scrollIntoViewIfNeeded(); await expect(page.getByRole('button', { name: 'Unavailable', exact: true })).toBeDisabled();
    ...
    await expect(page.getByLabel('Event stage')).toHaveValue('Discover'); await say(5, 'Inputs');
    await page.getByRole('heading', { name: '05 / Surfaces & feedback' }).scrollIntoViewIfNeeded(); await say(6, 'Feedback');
