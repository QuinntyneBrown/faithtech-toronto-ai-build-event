...
test('development estimate reflects saved work and stays independent of filters', async ({ page }) => {
  const dashboard = new DashboardPage(page); await dashboard.open(); await dashboard.expectDevelopment(64);
  await dashboard.inspect('L2-009'); await dashboard.save('review', 'Team controls implemented; acceptance pending.');
  await dashboard.expectDevelopment(65); await dashboard.expectRemaining(30);
  await dashboard.search('nothing-matches'); await dashboard.expectDevelopment(65);
  await page.reload(); await dashboard.expectDevelopment(65);
});
...
test('audit is searchable and filtering never changes the full-scope totals', async ({ page }) => {
  ...
  await dashboard.expectRemaining(30);
});
test('completion needs evidence, survives reload and can be reopened', async ({ page }) => {
  ...
  await dashboard.expectRemaining(29); await page.reload();
  await dashboard.expectRemaining(29); await dashboard.inspect('L2-009');
  await dashboard.save('progress', 'Reopened after a regression.'); await dashboard.expectRemaining(30);
});
test('export restores progress after browser storage is cleared', async ({ page }, testInfo) => {
  ...
  await page.evaluate(() => localStorage.clear()); await page.reload(); await dashboard.expectRemaining(30);
  await dashboard.import(path); await dashboard.expectRemaining(29);
});
...
test('invalid imports preserve current accepted reviews', async ({ page }) => {
  ...
  await dashboard.expectRemaining(29);
});
...
test('storage failure keeps reviews usable and warns to export', async ({ page }) => {
  ...
  await dashboard.save('done', 'Entry acceptance verified.'); await dashboard.expectRemaining(29);
