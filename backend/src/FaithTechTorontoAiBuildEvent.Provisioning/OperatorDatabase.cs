...
public static class OperatorDatabase
{
    public static async Task<int> VerifyConnectionAsync(string target)
    {
        try
        {
            ...
            var (server, database) = await ReadIdentityAsync(connection);
    ...
    private static async Task<(string Server, string Database)> ReadIdentityAsync(SqlConnection connection)
    {
        await using var command = new SqlCommand("SELECT @@SERVERNAME, DB_NAME();", connection) { CommandTimeout = 60 };
