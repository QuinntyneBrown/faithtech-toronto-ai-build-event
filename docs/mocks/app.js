...
export function render() {
  ...
  const content=adminEventScreens(screen,state)||eventScreens(screen,state)||projectScreens(screen,state,q.get('item'))||socialScreens(screen,state,q.get('item'),q.get('search')||'')||activityScreens(screen,state,q.get('item'))||title('DESIGN ARTIFACT','Screen not found','Return to the event entrance.',link('access','Event entrance'));
...
function openDialog(id,item) {const overlay=document.querySelector('#overlay');overlay.innerHTML=dialogMarkup(id,item,dialogContent(id,item)||adminDialogContent(id,item));if(overlay.innerHTML&&!overlay.open)overlay.showModal();}
