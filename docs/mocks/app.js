...
export function render() {
  ...
  const content=eventScreens(screen,state)||projectScreens(screen,state,q.get('item'))||socialScreens(screen,state,q.get('item'),q.get('search')||'')||activityScreens(screen,state,q.get('item'))||title('DESIGN ARTIFACT','Screen not found','Return to the event entrance.',link('access','Event entrance'));
...
document.addEventListener('submit',e=>{const f=e.target.closest('[data-form]');if(!f)return;e.preventDefault();const d=Object.fromEntries(new FormData(f));if(f.dataset.dialog==='draw'){draw(navigate);return;}const result=submitParticipant(f.dataset.dialog||f.dataset.form,d,f.dataset.item,navigate);if(typeof result==='string')f.querySelector('.form-error').textContent=result;});
