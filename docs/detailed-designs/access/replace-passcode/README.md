...
The direct operation is a parameterized RPC from a normal SQL client over encrypted, certificate-validated transport: procedure name `dbo.ReplaceAdminPasscode`, input parameter `@Passcode`, SQL type NVarChar(MAX). The operator selects the explicit server/database connection, confirms its resolved identity, opens the client's stored-procedure execution interface, binds the four-digit value as that input parameter, executes once, and records only the returned revision/time. The operator uses protected temporary input rather than a saved literal EXEC statement. SQL-client history, clipboard retention, parameter tracing, and diagnostic capture are disabled or protected for this operation; the result grid contains revision/time only. A client that records secret parameter values is not suitable for this protected workflow. The procedure performs all conversion without the API or companion CLI, so it works while the app is stopped.
