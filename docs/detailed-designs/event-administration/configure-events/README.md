...
`/admin/events` hosts `ConfigureEventsPage`; `/admin/events/:eventId` opens the editor. `IEventService` exposes `list`, `get`, `saveDraft`, `publish`, and `copy`. `EventSummary` contains ID, title, local start/end, timezone, publication state, and version. The list offers create/open/copy actions and a useful empty state.
...
`LogoAsset` stores validated decoded image bytes in SQL with the event reference, keeping publication and backup consistency in one durable store. Upload processing enforces shared size, format, MIME, and dimension rules before commit. The serving endpoint returns the detected media type and `X-Content-Type-Options: nosniff`; participant access checks publication and session scope.
