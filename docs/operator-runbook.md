...
3. Before admitting traffic, execute `dbo.InvalidatePrivateSessions` once through a protected, parameter-free SQL RPC, then rotate the administrator passcode through the supported SQL or CLI operation. This transaction revokes administrator sessions, participant sessions, and entry receipts; do not reopen traffic until both operations have succeeded.
